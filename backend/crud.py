...
def create_user_token(db: Session, user: models.User, token: str):
    """
    Kullanıcı için yeni bir token'ı veritabanına kaydeder.
    """
...
def get_user_by_token(db: Session, token: str):
    """
    Token'a göre kullanıcıyı bulan fonksiyon (Eski get_user_id'nin yerini alacak)
    """
...
def create_user_report(db: Session, report: schemas.ReportCreate, user_id: int):
    """
    Yeni raporun GCS linkini veritabanına kaydeder.
    """
...
def get_user_reports(db: Session, user_id: int, skip: int = 0, limit: int = 100):
    """
    Bir kullanıcının tüm raporlarını veritabanından listeler (Tespit 3.3' ü çözer)
    """
